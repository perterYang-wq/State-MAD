...
- `GenerationRequest`: complete effective generation identity: experiment/phase/condition/pair, Agent role, snapshot hash, rendered system/user prompts, visible message IDs and hashes, model/tokenizer identities and revisions, decoding configuration, seed, and `max_new_tokens`.
...
Canonical hashes use UTF-8 bytes of sorted-key, fixed-separator JSON with an explicit schema version. Lists whose order affects model input remain ordered; unordered sets are sorted before serialization. Hash algorithms and schema versions are stored with each hash.
...
| Cache implications | Scenario and answer mappings are not model cache entries. Their canonical hash is a required component of every downstream cache key. Recompilation with identical spec/seed must be byte-identical. |
...
| Cache implications | Rendered prompt bytes/hash and ordered visible-message IDs/content hashes enter the generation key. Awareness and no-peer template IDs/text must differ and therefore normally have distinct keys. A defensive assertion detects accidental byte-identical effective requests and forces shared reuse plus a `selected_degenerate` flag. |
...
| Cache implications | Parent and effective branch snapshot hashes enter keys. Branch labels alone do not alter a key unless effective input differs. Same effective request across siblings must converge to one key. |
...
| Input / output | Key includes scenario hash, experiment/phase, condition insofar as effective input differs, Agent ID/role, snapshot and prompt hashes, ordered visible IDs/content hashes, model/revisions, temperature/top-p/full decoding config, seed, and max tokens. Output is exact raw bytes plus derived fields/checksum and status. |
...
| Deterministic test | Counting fake backend: identical effective requests call it once and return byte-identical output thereafter; metadata-only differences excluded from effective input reuse the key; prompt/order/content changes alter it; corrupt entry and attempted duplicate commit fail without generation/overwrite. |
...
4. Identical effective request is generated once and replayed exactly; corruption fails closed.
