...
def build_manifest(config, artifacts, preflight, command, started_at, repo_probe=None):
    ...
    return {"schema":"state-mad-manifest-v1","repository":repo_probe,"config_hash":digest(config),"artifact_hashes":dict(artifacts),"preflight":preflight,"command":tuple(command),"started_at":started_at,"python":sys.version,"platform":platform.platform()}
