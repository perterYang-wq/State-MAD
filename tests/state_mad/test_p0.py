...
from state_mad.schema import BackendOutput, EffectiveGenerationIdentity, GenerationRequest, TokenUsage, canonical_bytes
...
class P0Tests(unittest.TestCase):
    ...
    def test_model_free_dry_run_lineage_tokens_and_gate(self):
        with tempfile.TemporaryDirectory() as d:
            backend=ScriptedBackend(self.ss.scenarios); result=run_e0_dry(CachedBackend(backend,MessageCache(d)))
            ...
            replay=run_e0_dry(CachedBackend(backend,MessageCache(d)))
